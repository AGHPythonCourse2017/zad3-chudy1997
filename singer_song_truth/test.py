...
from singer_song_truth.ConnectionException import ConnectionException
import re
import sys
import argparse
...
def get_titles_from_addr(addr,n):
...
def remove_wrong_titles(auth_title_list,title):
    res=[]
    for t in auth_title_list:
...
if __name__ == '__main__':
    ...
    parser.add_argument("sentence", nargs='+',type=lambda s: unicode(s, sys.getfilesystemencoding()),help="Sentence to be checked for truth\nUsage: ./test Leonard Cohen : Hallelujah")
    arg = ' '.join(parser.parse_args().sentence)

    auth_title=parse_inquiry(arg)
    addr = "http://www.tekstowo.pl/szukaj,wykonawca,"+auth_title[0].replace(' ','+')+",tytul,"+auth_title[1].replace(' ','+')+".html"

    auth_title_list=get_titles_from_addr(addr,30)
    auth_title_list=remove_wrong_titles(auth_title_list,auth_title[1])

    print(auth_title[0]+(' is the author of ' if judge_truth(auth_title_list,auth_title[0]) else ' probably is not the author of ')+auth_title[1])
